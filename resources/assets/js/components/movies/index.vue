<template>
  <div>
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-left">
          <v-chip
            :color="props.item.active ? 'purple accent-4' : 'purple darken-4'"
            text-color="white"
          >
        ...
        <td class="justify-left layout px-0">
          <v-tooltip bottom>
            <v-btn
              ...
              slot="activator"
              color="light-green"
              @click="openUpdateModal(props.item.id)"
            >
              <v-icon>edit</v-icon>
            </v-btn>
            <span>Editar</span>
          ...
          <v-tooltip bottom>
            <v-btn
              ...
              @click="openPickerTurnsModal(props.item.id)"
            >
          ...
          <v-tooltip bottom v-if="props.item.active">
            <v-btn
              ...
              @click="updateFieldData(props.item.id, false)"
            >
          ...
          <v-tooltip bottom v-else>
            <v-btn
              ...
              @click="updateFieldData(props.item.id, true)"
            >
    ...
    <picker-turns :show.sync="showPickerTurnsModal" :id.sync="elementId" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateFieldData: async function (id, state) {
      ...
      if (confirmation) {
        const response = await client.patch(`/movies/activator/${id}`, {
          active: state,
        });
        response &&
    ...
    openPickerTurnsModal: function (id) {
      this.showPickerTurnsModal = true;
      this.elementId = id;
    },
  },
  mounted() {
